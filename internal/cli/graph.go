...
func (a *App) graph(ctx context.Context, options Options, args []string) error {
	...
	if workingTree {
		...
		switch positional[0] {
		case "symbol":
			queryErr = renderSymbols(ctx, a.stdout, effective, target, positional[1], source)
		case "file":
			queryErr = renderFile(ctx, a.stdout, effective, target, positional[1], source)
		}
	...
	switch positional[0] {
	case "symbol":
		queryErr = renderSymbols(ctx, a.stdout, view, target, positional[1], source)
	case "file":
		queryErr = renderFile(ctx, a.stdout, view, target, positional[1], source)
	}
...
func parseGraphCommandOptions(options Options, args []string) (graphCommandOptions, []string, error) {
	result := graphCommandOptions{Committed: options.Committed}
...
func renderSymbols(ctx context.Context, output io.Writer, view graphReader, target repository.Target, query string, source graphSource) error {
...
func renderFile(ctx context.Context, output io.Writer, view graphReader, target repository.Target, query string, source graphSource) error {
